...
describe('workflow arguments', () => {
  ...
  it('should throw error for an unsupported type', async () => {
    ...
    try {
      await dehydrateWorkflowArguments(new Foo(), mockRunId, mockEncryptor, []);
    } catch (err_) {
...
describe('workflow return value', () => {
  it('should throw error for an unsupported type', async () => {
    ...
    let err: WorkflowRuntimeError | undefined;
    try {
      await dehydrateWorkflowReturnValue(new Foo(), mockRunId, mockEncryptor);
    } catch (err_) {
      err = err_ as WorkflowRuntimeError;
    }
    expect(err).toBeDefined();
...
describe('step return value', () => {
  it('should throw error for an unsupported type', async () => {
    ...
    let err: WorkflowRuntimeError | undefined;
    try {
      await dehydrateStepReturnValue(new Foo(), mockRunId, mockEncryptor, []);
    } catch (err_) {
      err = err_ as WorkflowRuntimeError;
    }
...
describe('custom class serialization', () => {
  ...
  it('should not serialize classes without WORKFLOW_SERIALIZE', async () => {
    ...
    await expect(
      dehydrateWorkflowArguments(instance, mockRunId, mockEncryptor, [])
    ).rejects.toThrow();
  });
  ...
  it('should throw error when classId is missing', async () => {
    ...
    let errorMessage = '';
    try {
      await dehydrateWorkflowArguments(instance, mockRunId, mockEncryptor, []);
    } catch (e: any) {
  ...
  it('should pass class as this context to WORKFLOW_SERIALIZE and WORKFLOW_DESERIALIZE', async () => {
    ...
    await dehydrateWorkflowArguments(counter2, mockRunId, mockEncryptor, []);
...
describe('format prefix system', () => {
  ...
  it('should throw error for data too short to contain format prefix', async () => {
    ...
    await expect(
      hydrateWorkflowArguments(tooShort, mockRunId, mockEncryptor, vmGlobalThis)
